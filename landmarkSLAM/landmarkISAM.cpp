/**
 * @file landmarkISAM.cpp
 * @brief Incremental SAM example using odometry measurements and bearing-range (laser) measurements
...
int main(int argc, char **argv){
  ...
  while(nextObs < dataset.size()){
    ...
    if(BetweenFactor<Pose2>::shared_ptr observation = 
      boost::dynamic_pointer_cast<BetweenFactor<Pose2> >(dataset[nextObs])){
  ...
  for(size_t step = firstPose; nextObs < dataset.size() && (lastStep == -1 || step <= lastStep); ++step){
    ...
    while(nextObs < dataset.size()){
      ...
      if(BetweenFactor<Pose2>::shared_ptr observation = 
        boost::dynamic_pointer_cast<BetweenFactor<Pose2> >(obsFactor)){
